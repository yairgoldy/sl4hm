...
#' @param index_case_prob probability that the index case is adult
#' @return a the result as a tibble including the number of adults, the number of children, the index case, and the pandemic duration (in days) of each family
#' @export
#' @import magrittr
...
create_family_structure_data <- function(seed = -1,n = 1000,index_case_prob = 0.8){
  ...
  dat <- tibble(size = sample(2:7,size = n,replace = T),
                        ...
                        first.is.adult = ifelse(adults == 0,FALSE,
                                                ...
                                                sample(x = c(TRUE,FALSE),size = n,prob= c(index_case_prob,1-index_case_prob),replace = T))),
                        day = 21) %>%
    dplyr::select(-size)
  return(dat)
...
#' Simulate the pandemic with in a household
#' @param seed seed for the simulation
...
#' @param T_max the length of the pandemic within the family
#' @param ssn the number of households to simuluate
#'
#' @return a tibble with the pandemic development over time by day
#' @export
#' @import magrittr
